...
export const createVariantSchema = z.object({
  ...
  selling_price : z.number({ error: "Selling price is required" }).min(0),
  stock         : z.number().min(0).optional().default(0),
  low_stock_alert: z.number().min(0).optional().default(5),
});
...
export const productQuerySchema = z.object({
  ...
  category_id: objectId.optional(),
  vendor_id  : objectId.optional(),
  has_variants: z.enum(["true", "false"]).optional().transform((v) => v === undefined ? undefined : v === "true"),
  ...
  sort_by    : z.enum(["name", "createdAt", "stock", "selling_price"]).optional().default("createdAt"),
